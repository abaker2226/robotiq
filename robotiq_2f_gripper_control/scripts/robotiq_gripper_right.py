...
class GripperCmdNode:
    def __init__(self):
        self.client = actionlib.SimpleActionClient('/right_gripper/command_robotiq_action', CommandRobotiqGripperAction)
        rospy.loginfo("[right_gripper] Waiting for action server...")
        self.client.wait_for_server()
        rospy.loginfo("[right_gripper] Connected to action server.")
    ...
    def send_goal(self, pos):
        ...
        goal.stop = False
        rospy.loginfo(f"[right_gripper] Sending goal: {pos}")
        self.client.send_goal_and_wait(goal)

if __name__ == "__main__":
    rospy.init_node("robotiq_gripper_right")
    GripperCmdNode()
    rospy.spin()
